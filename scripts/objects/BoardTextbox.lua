---@class BoardTextbox : Object
---@overload fun(...) : BoardTextbox
local BoardTextbox, super = Class(Object)

function BoardTextbox:init(x, y, width, height, instant)
...
function BoardTextbox:update()
	if self.text_con == 0 then
		if self.instant == true then
			self.y = self.endy
			self.text_con = 2
		else
			Assets.playSound("board/lift", 0.5, 1.2)
	...
	if self.text_con == 2 then
		if not self:isTyping() then
			...
			if not self.next_arrow then
				self.next_arrow = Sprite("sword/ui/arrow_nooutline")
...
function BoardTextbox:setSize(w, h)
    self.width, self.height = w or 0, h or 0
    self:updateTextBounds()
end

function BoardTextbox:setFont(font, size)
    if not font then
        self.font = self.default_font
        self.font_size = self.default_font_size
    else
...
function BoardTextbox:setAuto(auto)
    self.text.auto_advance = auto or false
end

function BoardTextbox:setAdvance(advance)
    self.text.can_advance = advance or false
end

function BoardTextbox:setSkippable(skippable)
    self.text.skippable = skippable or false
end

function BoardTextbox:setAlign(align)
    self.text.align = align or "left"
    self:updateTextBounds()
end

function BoardTextbox:setCallback(callback)
    self.advance_callback = callback
    self.text.advance_callback = callback
end

function BoardTextbox:resetFunctions()
    self.text.functions = {}
end

function BoardTextbox:addFunction(id, func)
    self.text:addFunction(id, func)
end

function BoardTextbox:setText(text, callback)
...
function BoardTextbox:getText()
    return self.text.text
end

function BoardTextbox:updateTextBounds()
    self.text.x = self.text_x
    self.text.width = self.width + self.wrap_add_w
end

function BoardTextbox:getBorder()
    if self.box.visible then
        return 0, 0
    end
end

function BoardTextbox:getDebugRectangle()
